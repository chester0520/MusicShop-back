...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (error.name === 'ValidationError') {
      ...
      res.status(400).send({ success: false, mesaage: message })
    } else if (error.name === 'MongoError' && error.code === '11000') {
      res.status(400).send({ success: false, mesaage: '帳號已存在' })
    } else {
      res.status(500).send({ success: false, mesaage: '伺服器錯誤' })
    }
  }
}
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res.status(400).send({ success: true, message: '帳號不存在' })
    }
  } catch (error) {
    res.status(500).send({ success: false, mesaage: '伺服器錯誤' })
  }
...
export const getuserinfo = async (req, res) => {
  try {
    res.status(200).send({
      success: true,
      message: '',
      result: { account: req.user.account, role: req.user.role, name: req.user.name, phone: req.user.phone, address: req.user.address, _id: req.user._id }
    })
  } catch (error) {
    console.log(error)
    res.status(500).send({ success: false, message: '伺服器錯誤' })
